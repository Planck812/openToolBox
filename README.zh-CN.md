...
>
> 1. **先双击应用**，让它被拦一次。这一步不能省：它会把应用记入被阻止列表，下一步正是从那里放行。
> 2. 打开**系统设置 → 隐私与安全性**，向下滚动到「安全性」，在 open-toolbox 的提示旁点**仍要打开**，并在随后的对话框中确认。
>
> 放行后 macOS 会记住，同一版本不再询问。
>
> 右键点击应用选「打开」有时也可以，但在较新的 macOS 上，一旦该应用已被拒绝过，这条路可能会静默失败。卡住的话请用上面的系统设置方式，那条路是可靠的。
>
> **不需要**关闭 Gatekeeper（`spctl --master-disable`）或开启「任何来源」。也请不要这么做 —— 为运行一个应用而大幅降低整机安全性并不划算。
