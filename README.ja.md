...
> このアプリは **Apple Developer ID で署名されていない**ため、初回起動時に「開発元を検証できないため開けません」と表示されてブロックされます。許可する手順：
>
> 1. **まずアプリをダブルクリック**してブロックさせます。この操作は省略できません — アプリがブロック済みリストに登録され、次の手順はそこから許可します。
> 2. **システム設定 → プライバシーとセキュリティ** を開き、「セキュリティ」まで下へスクロールし、open-toolbox の通知の横にある **このまま開く** をクリックして、続くダイアログで確認します。
>
> 一度許可すれば macOS が記憶し、同じバージョンでは再度尋ねられません。
>
> アプリを右クリックして「開く」を選ぶ方法でも通ることがありますが、新しい macOS では一度拒否された後だと無反応になる場合があります。うまくいかないときは上記のシステム設定からの手順をお使いください。
>
> Gatekeeper の無効化（`spctl --master-disable`）や「すべてのアプリケーションを許可」は**不要**です。アプリ 1 つのために端末全体の安全性を大きく下げることになるため、おすすめしません。
