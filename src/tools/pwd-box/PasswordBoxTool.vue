...
import { ChevronDown, Copy, Eye, EyeOff, KeyRound, Plus, RefreshCw, Trash2 } from 'lucide-vue-next';
...
const strengthLabel = (strength: PasswordStrength) =>
  t(`tools.pwd_box.gen_strength_${strength}`);

const visibleItems = computed(() => sortPasswordBoxItems(searchPasswordBoxItems(items.value, searchKeyword.value)));
...
const persistItems = async () => {
  try {
    ...
    const message = error instanceof Error && error.message ? error.message : t('tools.pwd_box.save_failed');
...
const toggleVisibility = (itemId: string) => {
  revealedIds.value = isPasswordVisible(itemId)
    ? revealedIds.value.filter((id) => id !== itemId)
    : [...revealedIds.value, itemId];
};
...
const copyPassword = async () => {
  ...
  appStore.showToast(
    t(ok ? 'tools.pwd_box.copy_success' : 'tools.pwd_box.copy_failed'),
...
const fillGeneratedPassword = async () => {
  ...
  if (!isPasswordVisible(selectedItem.value.id)) {
    toggleVisibility(selectedItem.value.id);
  }
...
const copyGeneratedPassword = async () => {
  ...
  appStore.showToast(
    t(ok ? 'tools.pwd_box.gen_copy_success' : 'tools.pwd_box.gen_copy_failed'),
    { type: ok ? 'success' : 'error' },
  );
};
...
onMounted(async () => {
  try {
    ...
    items.value = [];
    const message = error instanceof Error && error.message ? error.message : t('tools.pwd_box.load_failed');
    appStore.showToast(message, { type: 'error' });
  } finally {
    loaded.value = true;
    ensureSelectedItem();
...
<template>
  <div class="flex h-full min-h-0 flex-col bg-background text-foreground">
    ...
    <div ref="containerRef" class="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[minmax(240px,var(--panel-first-width,320px))_minmax(420px,1fr)]" :style="{ '--panel-first-width': firstPanelWidth === null ? undefined : `${firstPanelWidth}px` }">
      <aside ref="firstPanelRef" class="relative flex min-h-0 flex-col border-b border-border lg:border-b-0 lg:border-r">
        ...
        <div class="min-h-0 flex-1 overflow-auto p-3">
          ...
          <div v-else class="flex flex-col gap-3">
            <button
              ...
              :class="selectedItemId === item.id ? 'border-primary bg-primary/5' : 'border-border bg-card hover:border-primary/50 hover:bg-muted/40'"
              @click="selectItem(item.id)"
            >
              <div class="flex items-start justify-between gap-3">
                <div class="min-w-0 flex-1">
                  <div class="truncate text-sm font-medium">{{ item.site || t('tools.pwd_box.site_placeholder') }}</div>
                  <div class="mt-1 truncate text-xs text-muted-foreground">{{ item.username || t('tools.pwd_box.username_placeholder') }}</div>
                  <div class="mt-2 text-xs text-muted-foreground">
                    {{ isPasswordVisible(item.id) ? item.password : maskPassword(item.password) }}
                  </div>
                  <p class="mt-2 line-clamp-2 text-xs text-muted-foreground">{{ item.note || t('tools.pwd_box.note_placeholder') }}</p>
                </div>
        ...
        <div class="resizable-panel-divider" role="separator" :aria-label="t('tools.pwd_box.resize_aria')" aria-orientation="vertical" :aria-valuenow="firstPanelWidth ?? firstPanelRef?.clientWidth ?? 240" tabindex="0" @pointerdown.prevent="startResize" @keydown="handleResizeKeydown"></div>
      </aside>
      ...
      <section class="flex min-h-0 flex-col overflow-hidden">
        <div v-if="selectedItem" class="flex h-full min-h-0 flex-col gap-4 p-4">
          <div class="grid gap-4 md:grid-cols-2">
            ...
            <div class="flex flex-col gap-2">
              <div class="text-sm text-muted-foreground">{{ t('tools.pwd_box.username_label') }}</div>
              <input
          ...
          <div class="flex flex-col gap-3 rounded-xl border border-border bg-card p-4">
            <div class="flex items-center justify-between gap-3">
              <div class="text-sm text-muted-foreground">{{ t('tools.pwd_box.password_label') }}</div>
              <div class="flex items-center gap-2">
                ...
                >
                  ...
                  {{ isPasswordVisible(selectedItem.id) ? t('tools.pwd_box.hide_password') : t('tools.pwd_box.show_password') }}
                </button>
                <button
            ...
            <div data-testid="pwd-box-password-display" class="text-sm font-medium text-foreground">
              {{ isPasswordVisible(selectedItem.id) ? selectedItem.password : maskPassword(selectedItem.password) }}
            </div>
            ...
            <div class="rounded-lg border border-border/80 bg-muted/20">
              ...
              <div v-if="generatorOpen" class="flex flex-col gap-3 border-t border-border/80 p-3">
                ...
                <div class="grid grid-cols-2 gap-2 text-xs sm:grid-cols-3">
                  <label class="inline-flex items-center gap-2 rounded-md border border-border bg-background px-2 py-1.5">
                  ...
                  <label class="inline-flex items-center gap-2 rounded-md border border-border bg-background px-2 py-1.5 sm:col-span-2">
                ...
                <div class="rounded-md border border-dashed border-border bg-background/80 px-3 py-2">
